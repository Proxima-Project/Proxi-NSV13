// Оверрайдит дефолтные команды. Чтобы они выглядели более опрятно
...
/datum/tgs_chat_command/ircmanifest/Run(datum/tgs_chat_user/sender, params)
	...
	var/list/nano_crew_manifest = GLOB.data_core.get_manifest()
	...
	for(var/dept in nano_crew_manifest)
		...
		if(dept_list.len > 0)
			...
			var/depString
			switch(dept)
				if ("Command") depString = "Командование"
				if ("Misc") depString = "Неучтенный персонал"
				if ("Science") depString = "Научный отдел"
				if ("Security") depString = "Отдел безопасности"
				if ("Engineering") depString = "Инженерный отдел"
				if ("Medical") depString = "Медицинский отдел"
				if ("Supply") depString = "Отдел снабжения"
...
/datum/tgs_chat_command/adminwho/Run(datum/tgs_chat_user/sender, params)
	...
	for(var/adm in GLOB.admins)
		...
		if(can_investigate)
			if(C.is_afk())
				var/seconds = C.is_afk()/10
				line += " *(АФК - [round(seconds / 60)] minute\s, [seconds % 60] second\s)*"
			if(isobserver(C.mob))
				line += " - *Наблюдает*"
